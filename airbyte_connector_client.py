...
DEFAULT_BASE = "https://api.airbyte.com/v1"
...
class AirbyteClient:
    def __init__(self, api_key: str, base_url: str = ""):
        ...
        self.headers = {
            ...
            "User-Agent": "Imperal-Airbyte-Connector/1.0.0"
        }
    ...
    async def verify_auth(self) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                resp = await client.get(f"{self.base_url}/connectors", headers=self.headers)
    ...
    async def list_pipelines(self, limit: int = 20) -> list[dict[str, Any]]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/pipelines", headers=self.headers, params={"limit": limit})
            if resp.status_code == 200:
                data = resp.json()
                if isinstance(data, list): return data
                for k in ["data", "pipelines", "items", "results"]:
    ...
    async def get_pipeline(self, pipeline_id: str) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/pipelines/{pipeline_id}", headers=self.headers)
            if resp.status_code == 200:
                return resp.json()
